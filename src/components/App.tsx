...
import IsMovieContext, { TMovieContext } from './MovieContext';
...
const App = () => {
  ...
  const isMovieDetailsShown: TMovieContext = useMemo(
    () => [movieID, setMovieID],
    [movieID]
  );

  const chosenMovie = mockData.find(({ id }) => id === movieID);

  return (
    <>
      <IsMovieContext.Provider value={isMovieDetailsShown}>
        {movieID ? <MovieDetails movie={chosenMovie!} /> : <Header />}
        <FilterBar moviesData={mockData} />
      </IsMovieContext.Provider>
      <Footer />
    </>
  );
};
